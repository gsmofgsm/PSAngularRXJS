...
export class SupplierService {
  ...
  suppliersWithConcatMap$ = of(1, 5, 8)
    .pipe(
      tap(id => console.log(`ConcatMap source id ${id}`)),
      concatMap(id => this.http.get<Supplier>(`${this.suppliersUrl}/${id}`))
    );

  suppliersWithMergeMap$ = of(1, 5, 8)
    .pipe(
      tap(id => console.log(`MergeMap source id ${id}`)),
      mergeMap(id => this.http.get<Supplier>(`${this.suppliersUrl}/${id}`))
    );

  suppliersWithSwitchMap$ = of(1, 5, 8)
    .pipe(
      tap(id => console.log(`SwitchMap source id ${id}`)),
  ...
  constructor(private http: HttpClient) {
    ...
    this.suppliersWithMap$.subscribe(
      o => o.subscribe(item => console.log('Map double subscribe result', item))
    );
